  ...
  switch (req.method) {
    case 'GET':
      ...
      Post.findAll({ order: [['id', 'DESC']] }).then((posts) => {
        ...
        //レスアンカーを取得して配列にする
        const hrefNumber = post.content.match(/>>([0-9]+)/gi);
        //配列を文字列に変える：レスアンカー表記がない場合は空文字列
      if (hrefNumber === null){
        let hrefNumberStr = '';
        post.hrefNumberId = hrefNumberStr;
        post.hrefAnchor = hrefNumberStr; 
      }else{
        let hrefNumberStr = hrefNumber.toString();
        post.hrefNumberId = hrefNumberStr; 
        post.hrefAnchor = hrefNumberStr.replace(/>>/g, '#');
      }
